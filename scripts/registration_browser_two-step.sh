...
responseUpdateRegistrationFlow=$(curl -v -s -X POST \
  ...
  -H "Accept: application/json" \
  -H "Content-Type: application/json" \
  -d '{"csrf_token": "'$csrfToken'", "screen": "credential-selection", "method": "profile", "traits.email": "'$email'", "traits.phone": "'$phone'", "traits.firstname": "firstname", "traits.lastname": "lastname", "traits.nickname": "nickname"}' \
  "$actionUrl") 
...
responseUpdateRegistrationFlow=$(curl -v -s -X POST \
  ...
  -H "Accept: application/json" \
  -H "Content-Type: application/json" \
  -d '{"csrf_token": "'$csrfToken'", "method": "password", "password": "'$password'", "traits.email": "'$email'", "traits.phone": "'$phone'", "traits.firstname": "firstname", "traits.lastname": "lastname", "traits.nickname": "nickname"}' \
  "$actionUrl") 
echo $responseUpdateRegistrationFlow | jq
